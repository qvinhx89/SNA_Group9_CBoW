"""MAPR2026 v3 — Diagnostic graph-augmented flat baseline (LR own + 1-hop attr means).
...
pre-specified flat LR baseline for inferential claims.

Use ``--label-regime a0`` with ``regression_targets_a0.parquet`` and **without**
``--include-language`` for the symmetric A0 diagnostic (reviewer-facing one-hop
check vs HSCC). HSCC runs use ``--label-regime hscc`` (default) and typically
``--include-language``.

Outputs (defaults under PATHS.results_dir; override paths for A0):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Graph-augmented flat LR baseline (diagnostic; HSCC or A0)")
    parser.add_argument(
        "--label-regime",
        default="hscc",
        choices=["a0", "hscc"],
        help="Written to output CSV/JSON metadata. Use a0 for regression_targets_a0 (no --include-language).",
    )
