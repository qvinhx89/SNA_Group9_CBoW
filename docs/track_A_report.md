...
Hypothesis H1: Weighted-cascade IC tạo tín hiệu continuous có ý nghĩa cho regression (discriminative, không degenerate), dù nhãn binary top-10 có thể bất ổn ở boundary. Kỳ vọng là cv_score vượt ngưỡng readiness cho regression; nếu Jaccard nhãn thấp thì nghiêng về đặc tính cấu trúc của graph hơn là lỗi sampling thuần túy.
...
Hypothesis H3: One-hop và các structural metric sẽ tương quan khá cao với IC, nhưng không thay thế hoàn toàn IC. Two-hop bổ sung biến thiên beyond one-hop, và pattern tương quan không đồng đều giữa các vùng cấu trúc.
...
- Input chính: outputs/day1_benchmark/*.json, outputs/ic_feasibility/*.json  
