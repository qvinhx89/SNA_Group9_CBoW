...
def _learnability(
    ...
    def _fit_eval(x: np.ndarray) -> tuple[float, float]:
    ...
    g_rho, g_jac = _fit_eval(graph_feats)
    f_rho, f_jac = _fit_eval(full_feats)
    return {
...
def _compute_race_samples(
    ...
    for s in seeds:
        ...
        out.append(0.5 * float(rho_anchor) + 0.3 * float(learn["graph_plus_views_spearman"]) + 0.2 * float(learn["graph_plus_views_jaccard"]))
...
def _evaluate_formula(
    ...
    sender_used = sender_strength_views_centered if cfg.name == "hybrid_centered" else sender_strength
...
def main() -> None:
    ...
    if args.auto_race:
        ...
        for stage_idx, stage in enumerate(stages, start=1):
            ...
            for cfg in current_cfgs:
                ...
                print(
                    ...
                    f"rho={float(row['spearman_mean']):.4f}, ndcg_proxy={float(row['graph_plus_views_jaccard']):.4f}, runtime={float(row['runtime_sec']):.1f}s"
                )
        ...
        out["score_learnability"] = 0.7 * _minmax(out["graph_plus_views_spearman"]) + 0.3 * _minmax(out["delta_spearman"])
